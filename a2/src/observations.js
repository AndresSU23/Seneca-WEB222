...
function speciesCoordinates3(data) {
  for (const element of data.results) {
    console.log(
      `"${element.species_guess}" observed at coordinates (${element.location.replace(',', ', ')})`
    );
  }
}
...
function observationsByQualityGrade(data, qualityGrade) {
  ...
  if (
    ...
    throw new Error('Unvalid qualityGrade parameter provided.');
  }
  let matches = new Array();
  for (const element of data.results) {
    if (element.quality_grade === qualityGrade) matches.push(element);
  }
  return matches;
}
...
function observationsByQualityGrades(data, ...qualityGrades) {
  let matches = new Array();
  for (let grade of qualityGrades) {
    grade = typeof grade === 'string' ? grade.toLowerCase() : grade;
    if (grade !== 'research' && grade !== 'needs_id' && grade !== 'casual' && grade !== null) {
      throw new Error('Unvalid qualityGrade parameter provided.');
    }
    for (const element of data.results) {
      if (element.quality_grade === grade) matches.push(element);
    }
  }
  return matches;
}
...
function transformObservation(original) {
  function check(param, method = null, defaultVal = null) {
    if (param) {
      if (method) {
        return method(param);
      }
  ...
  function loadPhotos(photos) {
    let arr = new Array();
    photos.forEach((img) => {
      arr.push({ url: img.url, copyright: img.attribution });
    });
    return arr;
  }
  ...
  return {
    ...
    name: check(original.species_guess, (str) => str.toLowerCase()),
    isExtinct: check(
      original.conservation_status,
      (cStatus) => cStatus.status_name === 'extinct in the wild',
      false
    ),
    images: check(original.photos, loadPhotos, new Array()),
    observer: check(original.user, (login) => `${login.login_exact}@inaturalist.com`)
  };
}
...
function transformObservations(data) {
  ...
  data.results.forEach((entry) => {
    observations.push(transformObservation(entry));
  });
  return observations;
}
...
function getObservationsByTaxa(data, ...taxaNames) {
  ...
  taxaNames.forEach((name) => {
    arr = combineArray(
      arr,
      data.results.filter((entry) => entry.taxon.iconic_taxon_name === name)
    );
  });
  return arr;
}
...
function getObservationsByLocation(data, options = {}) {
  let location;
  function locationFilter(entry) {
    location = entry.location.split(',');
    if (options.lat) {
      if (options.lat.min) {
        return (
          location[0] >= options.lat.min &&
          location[0] <= options.lat.max &&
          location[1] >= options.lng.min &&
          location[1] <= options.lng.max
        );
      }

      return parseFloat(location[0]) === options.lat && parseFloat(location[1]) === options.lng;
    }
    return true;
  }
  return data.results.filter(locationFilter);
}
...
function getPlaceURLs(data) {
  let arr = [];
  data.results.forEach((entry) => {
    entry.place_ids.forEach((id) => {
      if (!arr.includes(id)) arr.push(id);
    });
  });
  for (let i = 0; i < arr.length; i++) {
    arr[i] = `https://www.inaturalist.org/observations?place_id=${arr[i]}`;
  }
  return arr;
}
...
function getSpeciesObservations(data) {
  let obj = {};
  data.results.forEach((entry) => {
    if (entry.taxon.name in obj) {
      obj[entry.taxon.name]++;
    } else {
      obj[entry.taxon.name] = 1;
    }
  });
  return obj;
}
...
function extractSpeciesNames(data) {
  let arr = [];
  data.results.forEach((entry) => {
    if (!arr.includes(entry.species_guess)) arr.push(entry.species_guess);
  });
  return arr;
}
...
function extractSpeciesNames2(data) {
  ...
  let set = new Set();
  data.results.forEach((entry) => {
    if (!set.has(entry.species_guess)) set.add(entry.species_guess);
  });
