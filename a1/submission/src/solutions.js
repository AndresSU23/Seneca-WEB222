...
function toCamelCase(name, uppercase) {
  ...
  for (let i = 1; i < name.length; i++) {
    if (name[i] != "") {
      ...
      name[i] = name[i][0] + name[i].substring(1, name[i].length).toLowerCase();
    }
  }

  if (uppercase) {
    name[0] = name[0].toUpperCase();
    name[0] = name[0][0] + name[0].substring(1, name[0].length).toLowerCase();
  }
...
function formatTimes(...values) {
  ...
  for (let i = 0; i < values.length - 1; i++) {
    values[values.length - 1] +=
      normalizeTime(values[i]) != null ? normalizeTime(values[i]) + ", " : "";
  }
  values[values.length - 1] = values[values.length - 1].substring(
    0,
    values[values.length - 1].length -
      (values[values.length - 1].length != 1 ? 2 : 0),
  );
