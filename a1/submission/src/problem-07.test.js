...
describe("Problem 7 - mimeTypeFromFilename() function", function () {
  ...
  test("correct MIME type for unknown extensions", function () {
    expect(mimeTypeFromFilename("file.exe")).toEqual(
      "application/octet-stream",
    );
    expect(mimeTypeFromFilename("library.dll")).toEqual(
      "application/octet-stream",
    );
  });

  test("absolute Unix file paths give correct MIME type", function () {
    expect(mimeTypeFromFilename("/public/site/www/cat.jpg")).toEqual(
      "image/jpeg",
    );
  });
  ...
  test("absolute Windows file paths give correct MIME type", function () {
    expect(
      mimeTypeFromFilename("C:\\Documents\\Seneca\\WEB222\\students.csv"),
    ).toEqual("text/csv");
  });

  test("spaces in path give correct MIME type", function () {
    expect(
      mimeTypeFromFilename("/this path/has quite a/few spaces/doc.txt"),
    ).toEqual("text/plain");
  });

  test("periods in path give correct MIME type", function () {
    expect(
      mimeTypeFromFilename("/this.path/has.quite.a/few.periods/dog.png"),
    ).toEqual("image/png");
  });
  ...
  test("filename with no extension should give unknown MIME type", function () {
    expect(mimeTypeFromFilename("/this/file/has/no/extension")).toEqual(
      "application/octet-stream",
    );
  });
  ...
  test("filename with unexpected extension should give unknown MIME type", function () {
    expect(
      mimeTypeFromFilename("/this/file/has/an/unknown/extension.cgi"),
