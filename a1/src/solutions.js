 ...
 *
 *      Name: David Andres Sanchez Umbaria
...
function parseDateString(value) {
  try {
    ...
    if (value[1].length == 2){
      ...
      if (value[0].length == 4 && value[2].length == 2){
        date.setFullYear(value[0]);
        date.setDate(value[2]);
        return date;
      }
      else if (value[2].length == 4 && value[0].length == 2){
...
function toDateString(value, format) {
  try {
    switch (format) {
      case "YYYY/MM/DD":
        return `${value.getFullYear()}/${((value.getMonth()+1 < 10) ? "0" : "") + (value.getMonth()+1)}/${((value.getDate() < 10) ? "0" : "") + (value.getDate())}`;
      case "DD/MM/YYYY":
        return `${((value.getDate() < 10) ? "0" : "") + (value.getDate())}/${((value.getMonth()+1 < 10) ? "0" : "") + (value.getMonth()+1)}/${value.getFullYear()}`;
      case "MM/DD/YYYY":
        return `${((value.getMonth()+1 < 10) ? "0" : "") + (value.getMonth()+1)}/${((value.getDate() < 10) ? "0" : "") + (value.getDate())}/${value.getFullYear()}`;
      default:
        throw new Error("M/D/Y")
    }
  } catch{
    throw new Error("M/D/Y");
  }
...
function normalizeTime(value) {
  ...
  if (value[1] < 0 || value[1] > 59 || value[2][0] < 0 || value[2][0] > 59){
    return null
  }
...
function formatTimes(...values) {
  ...
  for (let i = 0; i < values.length-1; i++) {
    values[values.length-1] += (normalizeTime(values[i]) != null) ? (normalizeTime(values[i]) + ", ") : "";
  }
  values[values.length-1] = values[values.length-1].substring(0, values[values.length-1].length - ((values[values.length-1].length != 1) ? 2 : 0));
